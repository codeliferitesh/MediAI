...
export const Appointments = () => {
  ...
  const fetchScheduleData = async () => {
    try {
      ...
      if (docRes.data.length > 0) setSelectedDocId(docRes.data[0].id);

      api.get<Patient[]>('/patients/directory').then(res => {
        setPatients(res.data);
        if (res.data.length > 0) setSelectedPatId(res.data[0].id);
      }).catch(() => {
        const mockPats = [
        ...
        if (mockPats.length > 0) setSelectedPatId(mockPats[0].id);
      });
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-extrabold text-slate-800 tracking-tight">Hospital Master Schedules</h2>
        <p className="text-slate-500 text-sm mt-1">Book consultations, cancel appointments, or manage check-in logs.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-xs p-6 space-y-5">
          <h3 className="text-base font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center gap-2">
            <Calendar className="h-5 w-5 text-medical-600" /> Administrative Booking
          </h3>
          ...
          {success && (
            <div className="bg-green-50 border border-green-200 text-green-700 p-3 rounded-lg text-sm font-semibold flex items-center gap-1.5 shadow-xs">
              <CheckCircle2 className="h-4.5 w-4.5" /> Appointment created successfully!
            </div>
          )}
          ...
          <form onSubmit={handleCreateAppointment} className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Appointment Date</label>
              <input
                type="date"
                required
                value={apptDate}
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="block w-full border border-slate-200 rounded-lg px-3 py-2 bg-slate-50 text-sm focus:outline-none focus:ring-2 focus:ring-medical-500 focus:bg-white text-slate-755"
                placeholder="Reason..."
              />
            </div>
            ...
            >
              {loading ? "Booking slot..." : "Schedule Appointment"}
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-100 shadow-xs p-6 space-y-5">
          <h3 className="text-base font-bold text-slate-800">Master Consultation Ledger</h3>
          ...
          {appointments.length === 0 ? (
            <div className="border border-dashed border-slate-150 rounded-xl p-8 text-center text-slate-400 min-h-[300px] flex flex-col items-center justify-center">
              ...
              <p className="font-semibold">No schedules recorded</p>
            </div>
          ) : (
            <div className="border border-slate-100 rounded-xl overflow-hidden shadow-2xs">
              <table className="min-w-full divide-y divide-slate-100">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="px-4 py-2.5 text-left text-xs font-bold text-slate-500 uppercase">Patient</th>
                    <th className="px-4 py-2.5 text-left text-xs font-bold text-slate-500 uppercase">Assigned Doctor</th>
                    <th className="px-4 py-2.5 text-left text-xs font-bold text-slate-500 uppercase">Date & Slot</th>
                    <th className="px-4 py-2.5 text-left text-xs font-bold text-slate-500 uppercase">Status</th>
                    <th className="px-4 py-2.5 text-right text-xs font-bold text-slate-500 uppercase">Actions</th>
                  </tr>
                </thead>
                  ...
                  {appointments.map((appt) => (
                    <tr key={appt.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-4 py-3 text-slate-650">
                        Dr. {appt.doctor?.profile?.full_name || "Staff Doctor"}
